...
- Back-to-back sponsor reads are still ads: if a second advertiser starts a few seconds after a CTA, label that complete read too, not only its URL line.
- Repeated midroll creatives are ads again each time they appear.

DECISION RULES:
1) Continuous ads: once an ad starts, follow it to its natural conclusion; include 1–5 second transitions. If a later line has a URL, pull in the preceding pitch for that same sponsor. After a short gap, the next advertiser in the same break is also an ad.
...
6) NEVER mark as ads: production credits ("produced by", "edited by", "fact-checked by"), host outros ("that's it for this episode", "that's next time"), class/show resumes ("All right, class", "back from the break"), cold-opens ("Welcome to…", "I'm your host", "For a generation of kids", date-in-history lines like "It is July the 1st, 1936" or "It's 7 in the morning"), or thank-yous to guests/professors.
7) MIXED SEGMENTS: if one timestamped line contains a sponsor CTA and then show content, output a start/end window covering ONLY the sponsor portion. Stop before "Welcome to", "I'm your host", or the first storytelling sentence. Do not label the cold-open.
